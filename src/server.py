...
def upload_batch_to_db(image_ids: list[str]):
    ...
    timer.start()
    queue_items = [task_queue.put_image(image_datas) for image_data in image_datas]
...
def api_upload_image(image_ids: list[str] = Body(...)):
    logger.info(f"[{threading.get_ident()}] ====== /api/upload/batch ======", extra={"color_message": f"[{threading.get_ident()}] ====== {click.style('/api/upload', bold=True)} ======"})
    resp_json = {"status": {"success": 0, "failed": 0}, "results": []}
    timer = SimpleTimer()
    timer.start()
